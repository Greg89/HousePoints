...
export function PlatformModerationQueue({ reports }: PlatformModerationReportList) {
  ...
  function act(reportId: string, action: "START_REVIEW" | "DISMISS" | "RESOLVE" | "REDACT_CONTENT" | "WARN_MEMBER" | "SUSPEND_MEMBER" | "RESTORE_MEMBER") {
    setMessage(null);
    startTransition(async () => {
      const result = await resolvePlatformModerationReport({ reportId, action, operatorNote: notes[reportId] || (action === "RESTORE_MEMBER" ? "Membership restored by platform operator." : "") });
