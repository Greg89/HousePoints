...
/**
 * A resolved house theme as a plain record of CSS custom property names to
 * string values. Kept framework-agnostic so both web (spread into a React
 * `CSSProperties` inline style) and mobile (translate to StyleSheet values)
 * can consume it.
 */
export type HouseThemeVars = Record<HouseThemeToken, string>;
