...
Status: in progress. `/members` and `/admin/context` now read active membership rows for user role and house assignment while preserving the existing response shapes. Notification recipient fanout now reads active membership rows for admin/owner and org-wide announcements.
