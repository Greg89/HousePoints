...
4. [-] Convert expected action failures to typed results. Current-user bootstrap, dashboard, and admin-context reads use typed response errors; point awards, profile updates, invite creation, house creation, house assignment, and onboarding mutations now return typed success/failure results for expected API failures while preserving warning logs; the unused legacy point-adjustment Server Action was removed so points now have one typed mutation path. Season start and rename mutations still need the same typed-result treatment. Auth account-link conflicts now return stable API codes and no longer surface as unhandled Prisma errors.
