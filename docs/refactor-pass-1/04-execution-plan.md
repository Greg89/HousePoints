...
4. [-] Convert expected action failures to typed results. Current-user bootstrap, dashboard, and admin-context reads use typed response errors; point awards, profile updates, and invite creation now return typed success/failure results for expected API failures while preserving warning logs; legacy point-adjustment submissions, onboarding mutations, house creation, and house assignment preserve typed API errors but still throw rather than return expected failures. Auth account-link conflicts now return stable API codes and no longer surface as unhandled Prisma errors.
