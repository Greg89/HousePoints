...
4. [-] Convert expected action failures to typed results. Dashboard and admin-context reads use typed response errors; point awards preserve typed API errors, while mutation actions still throw rather than return expected failures.
...
3. [-] Export response and error schemas from contracts. Current-user, dashboard, admin-context, and point-adjustment response schemas exist; remaining operations still need schemas.
4. [-] Parse all API responses in the web client. Current-user, dashboard, admin-context, and point-award responses are parsed; remaining mutation responses are not.
