...
- [-] Expected errors have stable codes and typed UI handling. The web response boundary preserves codes, account-link conflicts no longer become generic 500s, and point awards, profile updates, invite creation, plus house creation now return typed expected-failure results; remaining mutation actions still need the same treatment.
